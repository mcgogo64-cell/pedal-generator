    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-b from-zinc-50 to-zinc-100 dark:from-black dark:to-zinc-900">
      <main className="w-full max-w-5xl px-4 sm:px-6 py-6 sm:py-12">
        <div className="mb-6 sm:mb-8 flex items-start justify-between">
          <div>
            <h1 className="text-2xl sm:text-3xl font-semibold tracking-tight text-zinc-900 dark:text-zinc-50">{t.title}</h1>
            <p className="mt-2 text-sm sm:text-base text-zinc-600 dark:text-zinc-400">{t.subtitle}</p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <section className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 backdrop-blur p-4 sm:p-6">
            <h2 className="text-base sm:text-lg font-medium text-zinc-900 dark:text-zinc-100">{t.inputs}</h2>
            <div className="mt-4 space-y-4 sm:space-y-5">
              <div>
                <label className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  {t.rpm}
                  <InfoTooltip content={t.rpmTooltip} />
                </label>
                ...
                <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-500">{t.totalRotations}: {rotations.toLocaleString()}</div>
              </div>
              <div>
                <label className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  {t.duration}
                ...
                <div className="mt-2 grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-xs text-zinc-600 dark:text-zinc-400 mb-1">{t.minutes}</label>
                    <input 
                      type="number" 
                      min={0} 
                      step={0.5}
                  ...
                  <div>
                    <label className="block text-xs text-zinc-600 dark:text-zinc-400 mb-1">{t.seconds}</label>
                    <input 
                      type="number" 
                      min={0} 
                      value={durationSec} 
                ...
                <div className="mt-1 text-xs text-zinc-500 dark:text-zinc-500">
                  {t.total}: {Math.floor(durationSec / 60)}m {durationSec % 60}s
                </div>
              </div>
              <div>
                <label className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  {t.userWeight}
                  <InfoTooltip content={t.userWeightTooltip} />
                </label>
                <input 
                  type="number" 
                  min={30} 
              ...
              <div>
                <label className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  {t.resistance}
                  <InfoTooltip content={t.resistanceTooltip} />
                </label>
                <input 
                  type="range" 
                  min={1} 
                ...
                />
                <div className="mt-1 text-xs text-zinc-600 dark:text-zinc-400">{t.current}: <span className="font-semibold">{resistance}</span></div>
              </div>
              <div>
                <label className="flex items-center text-sm font-medium text-zinc-700 dark:text-zinc-300">
                  {t.batteryCapacity}
                  <InfoTooltip content={t.batteryCapacityTooltip} />
                </label>
                <input 
                  type="number" 
                  min={1} 
          ...
          <section className="rounded-2xl border border-zinc-200 dark:border-zinc-800 bg-white/70 dark:bg-zinc-900/60 backdrop-blur p-4 sm:p-6">
            <h2 className="text-base sm:text-lg font-medium text-zinc-900 dark:text-zinc-100">{t.metrics}</h2>
            <div className="mt-4 grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-xl bg-gradient-to-br from-rose-50 to-rose-100 dark:from-rose-900/30 dark:to-rose-800/30 border-2 border-rose-200 dark:border-rose-700 p-4 sm:p-5 sm:col-span-2">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs font-semibold text-rose-700 dark:text-rose-300 uppercase tracking-wide">{t.caloriesBurned}</div>
                  <div className="text-xs text-rose-600 dark:text-rose-400">{t.met} {(3 + 0.7 * resistance).toFixed(1)}</div>
                </div>
                ...
                <div className="mt-2 text-xs sm:text-sm text-rose-600/80 dark:text-rose-400/80">
                  {t.basedOn} ({weightKg} kg) {t.andResistance}
                </div>
              </div>
              ...
              <div className="rounded-xl bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-900/30 dark:to-blue-800/30 border-2 border-blue-200 dark:border-blue-700 p-3 sm:p-4">
                <div className="text-xs font-semibold text-blue-700 dark:text-blue-300 uppercase tracking-wide mb-2">{t.energy}</div>
              ...
              <div className="rounded-xl bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-900/30 dark:to-emerald-800/30 border-2 border-emerald-200 dark:border-emerald-700 p-3 sm:p-4">
                <div className="text-xs font-semibold text-emerald-700 dark:text-emerald-300 uppercase tracking-wide mb-2">{t.batteryRestored}</div>
                ...
                <div className="text-xs sm:text-sm text-emerald-600/70 dark:text-emerald-400/70 mt-1">{t.capacity}: {batteryCapacityWh} Wh</div>
                {animPct > 0 && (
                  <div className="mt-2 text-xs sm:text-sm text-emerald-600/80 dark:text-emerald-400/80 italic">
                    {t.enoughToCharge}{Math.round((animWh / 12) * 100)}%
                  </div>
                )}
              </div>
              ...
              <div className="rounded-xl bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-900/30 dark:to-indigo-800/30 border-2 border-indigo-200 dark:border-indigo-700 p-3 sm:p-4 sm:col-span-2">
                <div className="text-xs font-semibold text-indigo-700 dark:text-indigo-300 uppercase tracking-wide mb-2">{t.averagePower}</div>
                ...
                <div className="text-xs sm:text-sm text-indigo-600/70 dark:text-indigo-400/70 mt-1">{t.durationLabel}: {Math.round(durationSec / 60)} {t.minutes.toLowerCase()}</div>
              </div>
            </div>
          </section>
        </div>
        ...
        <div className="mt-6 sm:mt-8 rounded-lg bg-zinc-100 dark:bg-zinc-800/50 border border-zinc-200 dark:border-zinc-700 p-3 sm:p-4">
          <p className="text-xs sm:text-sm text-zinc-700 dark:text-zinc-300 leading-relaxed">
            <strong className="font-semibold">{t.note}</strong> {t.noteText}
          </p>
        </div>
      </main>
    </div>
